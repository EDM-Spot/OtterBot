module.exports = function (bot, filename) {
	bot.commands.register("clear", filename, ["lag", "laggy"], 1000, true, {type: "per_use", duration: 60}, function (raw_data, command) {
		let user;
		...
		return bot.plug.chat(bot.utils.replace(bot.lang.commands.default, {
			...
			user: raw_data.un,
			message: `${bot.lang.commands.clear} ${user || ""}`
		})).delay(6e4).call("delete");
	}, {
		parameters: "[@username]",
		description: "Informs about the uses of /clear and /cap on plug.dj, to help reduce lag."
	});
};