module.exports = function (bot, filename) {
	bot.commands.register("waitlist", filename, ["wl"], 1000, true, {type: "per_use", duration: 60}, function (raw_data, command) {
		let user;
		...
		return bot.plug.chat(bot.utils.replace(bot.lang.commands.default, {
			...
			user: raw_data.un,
			message: `${bot.lang.commands.waitlist} ${user || ""}`
		})).delay(6e4).call("delete");
	}, {
		parameters: "[@username]",
		description: "Informs the users on how to queue up a song."
	});
};