module.exports = function (bot, filename) {
	bot.commands.register("app", filename, ["mobileapp", "mobile"], 0, true, {type: "per_use", duration: 60}, function (raw_data, command) {
		let user;
		...
		return bot.plug.chat(bot.utils.replace(bot.lang.commands.default, {
			...
			user: raw_data.un,
			message: `${bot.lang.commands.app} ${user || ""}`
		})).delay(6e4).call("delete");
	}, {
		parameters: "[@username]",
		description: "Shares links to the official plug.dj Mobile Apps."
	});
};